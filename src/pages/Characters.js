...
export default () => {
    ...
    const {error, isLoading, elements, statusCode, pagesQuantity} = useMarvelFetch(apiURL, {})
    ...
    useEffect(() => {
        setApiURL(buildCharactersURL({...filters, limit: LIMIT, offset: LIMIT * selectedPage}))
        ...
        <div className="">
            <SectionHeader>
                ...
                <h3 className="text-xl font-bold py-2 text-gray-100 cursor-pointer"
                    onClick={toggleIsFilterHidden}>Filters
                    <span>
                        <svg xmlns="http://www.w3.org/2000/svg"
                             className={`w-6 inline transition duration-200 ease-in-out transform ${!isFilterHidden ? 'rotate-180' : null}`}
            ...
            <div className="container mx-auto py-5">
                ...
                {!isLoading && statusCode !== 200 ?
                    <ErrorMessage title="Application Error" actionLink={"/"} actionTitle="Go home"/>
                : null}
                ...
                {elements && !isLoading && statusCode === 200 ?
                    <>
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-x-4 gap-y-8 px-2">
                            {elements.length === 0 ?
                                <GenericMessage title="No elements" subTitle="Try a different search"/>
                            : null}

                            {elements.map(element => {
                                    let extras = []
                                    if (element.comics && element.comics.available > 0) {
                                        extras.push({className: 'bg-red-500', title: `${element.comics.available} comics`})
                                    }
                                    ...
                                    if (element.series && element.series.available > 0) {
                                        extras.push({className: 'bg-blue-500', title: `${element.series.available} series`})
                                    }
                                    return (
                                        <ElementCard key={nanoid()}
                                                     image={`${element.thumbnail.path}.${element.thumbnail.extension}`}
                                                     link={`/characters/${element.id}`} title={element.name} extras={extras}/>
                                    )
                                }
                            )}
                        </div>
                        ...
                        {elements.length > 0 ?
                            <div className="container mx-auto pb-20 py-10">
                                <Pagination handlePageClick={handlePageClick} pagesQuantity={pagesQuantity}
