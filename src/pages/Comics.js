                    ...
                    <>
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-x-4 gap-y-8 px-2">
                            ...
                            {elements.map(element => {
                                    let extras = []
                                    if (element.characters && element.characters.available > 0){
                                        extras.push({className: 'bg-red-500', title: `${element.characters.available} characters`})
                                    }
                                    if (element.format){
                                        extras.push({className: 'bg-green-500', title: element.format})
                                    }
                                    return (
                                        <ElementCard key={nanoid()} image={`${element.thumbnail.path}.${element.thumbnail.extension}`}
                                                     link={`/comics/${element.id}`} title={element.title} extras={extras} />
                                    )
                                }
                            )}
                        </div>
